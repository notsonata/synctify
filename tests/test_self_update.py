...
def _release_response(version: str = "1.2.1") -> dict[str, object]:
    return {
...
def _available_release() -> ReleaseInfo:
    return ReleaseInfo(
        version="1.2.1",
        tag="v1.2.1",
        asset_name="synctify-1.2.1-macos.zip",
...
def test_find_update_uses_latest_stable_macos_release_asset() -> None:
    ...
    with httpx.Client(transport=httpx.MockTransport(handler)) as client:
        release = self_update.find_update("1.2.0", client)

    assert release is not None
    assert release.version == "1.2.1"
    assert release.tag == "v1.2.1"
    assert release.asset_name == "synctify-1.2.1-macos.zip"


def test_find_update_returns_none_when_current_is_latest() -> None:
    def handler(_request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=_release_response("1.2.0"))

    with httpx.Client(transport=httpx.MockTransport(handler)) as client:
        assert self_update.find_update("1.2.0", client) is None
...
def test_automatic_update_checks_on_every_installed_invocation(
    ...
    assert calls == 2
    assert first.release is not None and first.release.version == "1.2.1"
    assert second.release is not None and second.release.version == "1.2.1"
...
def test_upgrade_check_reports_without_installing(
    ...
    assert "Synctify 1.2.1 is available (current: 1.2.0)." in result.output
...
def test_explicit_upgrade_installs_available_release(
    ...
    assert result.exit_code == 0
    assert installed == ["1.2.1"]
    assert "Synctify upgraded to 1.2.1." in result.output
...
def test_noninteractive_cli_notifies_without_prompting(
    ...
    assert "Synctify application 1.2.1 is available" in result.output
...
def test_interactive_default_policy_prompts_before_application_upgrade(
    ...
    assert prompts == [
        "Synctify application 1.2.1 is available (current: 1.2.0). Upgrade now?"
    ]
